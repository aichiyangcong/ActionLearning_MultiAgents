...
- config.py: LLM 配置管理，封装 Anthropic API 配置和审查参数，支持 coach/evaluator/observer 三种 agent_type
- nested_chat.py: 返回 AG2 NestedChatTarget 实例，配置 Coach-Evaluator 审查循环（chat_queue 格式），可直接用于 OnCondition
- orchestrator.py: Phase 2 中枢编排器，数据流 User → Observer(FunctionTarget) → Coach → NestedChat(Evaluator)，集成三层记忆系统
...
## 数据流
```
User Input → UserProxy → Observer(FunctionTarget, 提取L1) → Coach → NestedChat(Evaluator) → 输出
                                    ↓
                          更新 ContextVariables.cognitive_state
```
